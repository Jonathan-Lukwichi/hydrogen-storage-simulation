    ...
    <div className="hh-art" style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg-0)' }}>
        <window.HHTopBar title="Validation Studio" subtitle="Mesh sweep · t = 3600s" actions={
          <><button className="hh-btn hh-btn-ghost" style={{ padding: '8px 14px', fontSize: 12 }}>Export audit</button>
            <button className="hh-btn hh-btn-primary" style={{ padding: '8px 16px', fontSize: 12 }}>▶ Re-run sweep</button></>
        }/>
          ...
          <div className="hh-card" style={{ padding: 22 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>Concentration error % vs elements (log scale)</div>
              </div>
              <window.HHPillRow items={['Concentration', 'Temp', 'Stress']} active="Concentration" />
            </div>
            <window.HHLine
              width={900} height={220}
              series={[
                { name: 'Numerical error %', points: [[1, 7.9], [3, 4.8], [6, 3.0], [10, 2.0], [14, 1.1], [20, 0.6]], color: '#00E5FF', strokeWidth: 2, markers: true, areas: true },
